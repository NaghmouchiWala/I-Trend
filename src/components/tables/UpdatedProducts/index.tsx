...
import { ChevronLeft, ChevronRight, CircleArrowDown, CircleArrowRight, CircleArrowUp, Home, RotateCcw, ArrowBigDown, ArrowBigUp } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useBaseProducts, useMytekProducts, useTunisianetProducts, useUpdatedProducts, useUpdatedProductss } from '@/hooks/products';
...
export function UpdatedProducts({ setSelectedProduct }: { setSelectedProduct: (product: ProductRo) => void }) {
 ...
 // Replace useGetProducts with:
 const useGetProducts = selectedCompany === 'tunisianet' 
 ? useTunisianetProducts({
     ...
     modification_date_min: selectedMinDate.toISOString().split("T")[0],
     modification_date_max: selectedMaxDate.toISOString().split("T")[0],
 ...
 : useMytekProducts({
  ...
  const price_change_counts = useGetProducts.data?.price_change_counts;
    ...
    <Card>
      <CardHeader className="flex flex-col gap-4">
        <div className="flex justify-between items-center">
          <div>
            <CardTitle>Produits Modifiés</CardTitle>
            <CardDescription>
              Liste des produits récemment modifiés.
            </CardDescription>
          </div>
          <div>
            <Select
        ...
        {isCompanyImplemented && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <StatCard title="Produits modifiés" number={totalProducts ?? 0}>
              <Home />
            </StatCard>
            {stock_counts && (
              <>
            ...
            {price_change_counts && (
              <>
                <StatCard
                    ...
                    number={price_change_counts.nbre_price_increase}
                  >
                    <ArrowBigUp />
                  </StatCard>
                  <StatCard
                    title="Prix en baisse"
                    number={price_change_counts.nbre_price_decrease}
                  >
                    <ArrowBigDown />
                  </StatCard>
                </>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="hidden md:table-cell">Ancien Prix</TableHead>
      ...
      {isCompanyImplemented && (products?.length ?? 0) > 0 ? (
        <CardFooter>
          <form className="flex items-center w-full justify-between">
            <div className="flex items-center gap-2">
              <div className="text-xs text-muted-foreground">
                <strong>
                  {Math.max(0, Math.min(offset - productsPerPage, totalProducts ?? 0) + 1)}-{offset}
                </strong>{' '}
